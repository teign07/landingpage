...
(() => {
  ...
  const route=document.createElement('nav');route.setAttribute('aria-label','A first reading');route.innerHTML=[['first-arrival','Begin here'],['first-how','How the app works'],['first-write','Leave a sentence'],['first-paper','Paper Books & prices'],['first-world','The Academy & Radio'],['first-privacy','What stays yours'],['first-parting','Take a Page with you']].map(([id,text])=>`<a href="#${id}"><strong>${text}</strong><span aria-hidden="true">↗</span></a>`).join('');details.before(route);
